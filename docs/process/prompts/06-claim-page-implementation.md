...
Read the event-key address returned by the app callback and show eligibility for it from `eligible.json` and the corresponding proof file. Ask for a recipient address and obtain a purpose-02 signature through the app link; its body contains the 32-byte chain ID, 20-byte claim-contract address and 20-byte recipient. The callback fields are `sig`, `k`, `a` and `st`. Submit `claim(snapshotId, eventKeyAddress, proof, recipient, appSignature)` from an injected wallet, which need not be the event key.
